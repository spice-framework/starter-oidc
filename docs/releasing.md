...
`STARTER_OIDC_RELEASE_SIGNING_KEY` secret with owner-only permissions, invokes
...
above deliberately continue to invoke `cmd/starter-oidc-release` and emit its
...
repository, GitHub's protected `spice-framework/starter-oidc` release channel
