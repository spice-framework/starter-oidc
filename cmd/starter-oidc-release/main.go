// Command starter-oidc-release builds deterministic signed library artifacts.
package main
...
import (
	...
	starterrelease "github.com/spice-framework/starter-oidc/internal/release"
)
...
func run(ctx context.Context, arguments []string, stdout, stderr io.Writer) int {
	flags := flag.NewFlagSet("starter-oidc-release", flag.ContinueOnError)
	...
	if flags.NArg() != 0 || version == "" {
		return writeExit(stderr, 2, "starter-oidc-release: -version is required and positional arguments are not accepted\n")
	}
	if err := starterrelease.ValidateVersion(version); err != nil {
		return writeExit(stderr, 2, "starter-oidc-release: %v\n", err)
	}
	if rehearsal && signingKey != "" {
		return writeExit(stderr, 2, "starter-oidc-release: rehearsal is always unsigned; remove -signing-key\n")
	}
	commit, epoch, err := sourceIdentity(ctx, root, explicitEpoch)
	if err != nil {
		return writeExit(stderr, 1, "starter-oidc-release: %v\n", err)
	}
	if !rehearsal {
	...
	if !rehearsal {
		if signingKey == "" {
			return writeExit(stderr, 1, "starter-oidc-release: -signing-key is required outside rehearsal\n")
		}
		key, err = readKey(signingKey)
		if err != nil {
			return writeExit(stderr, 1, "starter-oidc-release: read signing key: %v\n", err)
		}
	}
	...
	if _, err := fmt.Fprintf(
		stdout,
		"starter-oidc %s: created %d artifact(s) in %s\n",
		version,
		len(result.Files),
		result.OutputDir,
	); err != nil {
