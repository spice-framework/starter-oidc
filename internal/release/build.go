...
func Build(ctx context.Context, config Config) (result Result, resultErr error) {
	...
	staging, err := os.MkdirTemp(filepath.Dir(normalized.OutputDir), ".starter-oidc-release-*")
	if err != nil {
...
func buildArtifacts(ctx context.Context, config Config, staging string) ([]string, error) {
	...
	archiveName := "starter-oidc_" + versionName + "_source.tar.gz"
	...
	sbomName := "starter-oidc_" + versionName + "_sbom.spdx.json"
