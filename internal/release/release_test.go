...
func TestBuildIsDeterministicAndVerifiable(t *testing.T) {
	...
	for _, output := range outputs {
		...
		want := []string{
			...
			"starter-oidc_1.2.3-rc.1+build.7_sbom.spdx.json",
			"starter-oidc_1.2.3-rc.1+build.7_source.tar.gz",
		}
	...
	verifySourceArchive(t, filepath.Join(outputs[0], "starter-oidc_1.2.3-rc.1+build.7_source.tar.gz"), epoch)
	verifySBOM(t, filepath.Join(outputs[0], "starter-oidc_1.2.3-rc.1+build.7_sbom.spdx.json"), commit, epoch)
}
...
func TestBuildUsesCommittedSourceAndSupportsUnsignedRehearsal(t *testing.T) {
	...
	archive := readArchive(t, filepath.Join(output, "starter-oidc_0.0.0-rehearsal_source.tar.gz"))
	if got := string(archive["starter-oidc_0.0.0-rehearsal/tracked.txt"]); got != committedContent {
	...
	if _, found := archive["starter-oidc_0.0.0-rehearsal/untracked-secret.txt"]; found {
...
func TestBuildRejectsStaleCommittedGraphs(t *testing.T) {
	...
	t.Run("stale vendor", func(t *testing.T) {
		...
		for _, entry := range entries {
			if strings.HasPrefix(entry.Name(), ".starter-oidc-release-") {
...
func verifySourceArchive(t *testing.T, name string, epoch time.Time) {
	...
	for {
		...
		found = true
		if !strings.HasPrefix(header.Name, "starter-oidc_1.2.3-rc.1+build.7/") || filepath.IsAbs(header.Name) {
...
func verifySBOM(t *testing.T, name, commit string, epoch time.Time) {
	...
	if strings.Contains(string(readFile(t, name)), `:\`) || !strings.Contains(document.DocumentNamespace, "starter-oidc") {
