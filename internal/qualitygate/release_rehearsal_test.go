...
func writeReleaseFixture(t *testing.T) (string, map[string][sha256.Size]byte) {
	...
	sbom := releaseSBOM{
		...
		Name:              "starter-oidc " + rehearsalVersion,
		DocumentNamespace: "https://github.com/spice-framework/starter-oidc/releases/" + rehearsalVersion + "/spdx/v1/" + strings.Repeat("a", sha256.Size*2),
		...
		Packages:          []map[string]any{{"SPDXID": "SPDXRef-Package", "name": "starter-oidc"}},
